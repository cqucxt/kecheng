...
def write_in_file(str,str_mess):
    try:
...
def get_mode():
    ...
    if mode == '1':
        ...
        message=read_out_file('DES.txt')
        print("请输入你的秘钥：")
        key = input().replace(' ','')
        s = all_message_encrypt(message,key)
        out_mess = bin2str(s)
        print("加密过后的内容:"+ out_mess)
        write_in_file('DES1.txt',out_mess)
        #print(type(out_mess))
        ...
        print("请输入你的秘钥：")
        key = input().replace(' ', '')
        message = read_out_file('DES1.txt')
        s = all_message_decrypt(message, key)
        #out_mess = bin2str(s)

        print("解密后的信息："+ s)
        write_in_file('DES2.txt',s)
    else:
        print("请重新输入！")
